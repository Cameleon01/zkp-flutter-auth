...
class ZKAuthClient {
  ...
  Future<AuthResult> authenticate(String username) async {
    ZKLogger.banner('AUTHENTIFICATION  ›  $username');

    try {
      //  ÉTAPE 1 : Vérifier l'enrôlement
      ZKLogger.step(1, 'Vérification enrôlement');

      final isEnrolled = await _storage.isEnrolled(username);
      ZKLogger.valueShort('enrolled', isEnrolled.toString());

      if (!isEnrolled) {
        ZKLogger.error('Utilisateur non enrôlé');
        return AuthResult(
          success: false,
          error: 'Utilisateur non enrôlé. Veuillez vous inscrire d\'abord.',
        );
      }
      ZKLogger.success('Utilisateur enrôlé');

      //  ÉTAPE 2 : Récupérer la clé privée
      ZKLogger.step(2, 'Récupération clé privée (Android Keystore)');

      final privateKey = await _storage.getPrivateKey(username);

      if (privateKey == null) {
        ZKLogger.error('Clé privée introuvable dans le stockage sécurisé');
        return AuthResult(
          success: false,
          error: 'Clé privée introuvable. Veuillez vous ré-enrôler.',
        );
      }

      ZKLogger.success('Clé privée récupérée');
      ZKLogger.value('privKey (aperçu)', privateKey, truncate: true);
      ZKLogger.valueShort('longueur',
          '${privateKey.length} chars (${privateKey.length ~/ 2} bytes)');

      //  ÉTAPE 3 : Demander un challenge au serveur
      ZKLogger.step(3, 'Requête challenge → serveur');

      final challengeUrl = '$baseUrl/api/auth/challenge/?username=$username';
      ZKLogger.request('GET', challengeUrl);
      ...
      if (challengeResponse.statusCode != 200) {
        ZKLogger.error('Challenge refusé par le serveur');
        ZKLogger.valueShort('body', challengeResponse.body);
      ...
      ZKLogger.success('Challenge reçu');
      ZKLogger.value('challenge', challenge);
      ZKLogger.valueShort('longueur', '${challenge.length} chars');

      //  ÉTAPE 4 : Sauvegarder le challenge
      ZKLogger.step(4, 'Stockage temporaire du challenge');

      await _storage.saveChallenge(challenge);
      ZKLogger.success('Challenge sauvegardé en mémoire temporaire');

      //  ÉTAPE 5 : Générer la preuve ZK Schnorr
      ZKLogger.step(5, 'Génération preuve Zero-Knowledge (Schnorr)');
      ZKLogger.info('Protocole : k → R=k·G → e=H(R‖challenge) → s=k+e·privKey');
      ...
      ZKLogger.cryptoBlock(
        challenge: challenge,
        r: proof.r,
        s: proof.s,
      );

      //  ÉTAPE 6 : Envoyer la preuve au serveur
      ZKLogger.step(6, 'Envoi preuve → serveur (aucun secret transmis)');

      final authUrl = '$baseUrl/api/auth/authenticate/';
      ...
      ZKLogger.request('POST', authUrl);
      ZKLogger.value('payload.username', username, truncate: false);
      ZKLogger.value('payload.proof.r', proof.r);
      ZKLogger.value('payload.proof.s', proof.s);
      ZKLogger.value('payload.proof.challenge', challenge);
      ...
      ZKLogger.step(7, 'Traitement réponse serveur');

      if (authResponse.statusCode == 200) {
        final authData = json.decode(authResponse.body);

        //  le serveur retourne 'token' (pas 'access_token')
        final token = authData['token'] as String?;
        final message = authData['message'] as String?;

        ZKLogger.valueShort('message serveur', message ?? '(vide)');
        ZKLogger.valueShort('token présent', (token != null).toString());

        //  ÉTAPE 8 : Sauvegarder le token de session
        ZKLogger.step(8, 'Sauvegarde token de session');

        if (token != null) {
          await _storage.saveSessionToken(token);
          await _storage.saveAuthTimestamp(DateTime.now());
          ZKLogger.value('token (aperçu)', token);
          ZKLogger.success('Token sauvegardé');
        } else {
          ZKLogger.warning('Aucun token dans la réponse (vérifier le backend)');
        }

        //  ÉTAPE 9 : Définir l'utilisateur courant
        ZKLogger.step(9, 'Mise à jour utilisateur courant');
        await _storage.setCurrentUsername(username);
        ZKLogger.success('Utilisateur courant → $username');

        ZKLogger.authResult(
          success: true,
          token: token,
          httpStatus: authResponse.statusCode,
        );
        ...
        // Échec côté serveur
        String errorMsg = 'Authentification échouée';
        try {
          final errData = json.decode(authResponse.body);
      ...
      ZKLogger.divider();
      ZKLogger.error('EXCEPTION inattendue : $e');
      ZKLogger.valueFull('stackTrace', stackTrace.toString());
      ZKLogger.divider();
        ...
        if (reEnrollData['success'] == true) {
          ...
          try {
            ...
            if (driveId != null) {
              ...
              print('[RESTORE]  Échec mise à jour Drive');
            }
            ...
            if (serverResp.statusCode == 200 || serverResp.statusCode == 201) {
              ...
              print(
                  '[RESTORE]  Échec mise à jour serveur: ${serverResp.statusCode}');
            }
          } catch (backupErr) {
            // La restauration est réussie même si le re-backup échoue
            print('[RESTORE]  Re-backup échoué (non bloquant): $backupErr');
          }
          // ====================================================
    ...
    try {
      ...
      if (token != null) {
        ...
        if (response.statusCode == 200) {
          ...
          print('[LOGOUT]  Serveur: ${response.statusCode}');
        }
      }
    } catch (e) {
      // Même si le serveur est injoignable, on déconnecte localement
      print('[LOGOUT]  Erreur serveur (déconnexion locale quand même): $e');
    }
  ...
  Future<bool> _authenticateForSensitiveOperation({
    ...
    try {
      ...
      if (!canCheckBiometrics && !isDeviceSupported) {
        print('[AUTH]  Aucune sécurité disponible');
        return false;
      }
